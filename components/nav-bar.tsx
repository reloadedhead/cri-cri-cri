"use client";

import Image from "next/image";
import Link from "next/link";
import { useParams, usePathname } from "next/navigation";

export function Navbar() {
	const params = useParams();
	const pathname = usePathname();

	const isCharacterSheet = pathname?.startsWith("/characters/") && !!params?.id;

	return (
		<nav className="fixed left-0 top-0 z-20 mx-auto flex h-16 w-full items-center border-b-4 border-border bg-secondary-background px-5">
			<div className="mx-auto flex w-full text-foreground max-w-full items-center justify-between">
				<div className="flex items-center xl:gap-10 gap-10">
					<Link className="flex items-center gap-4" href="/">
						<Image
							src="/icons/dice.svg"
							alt="D20"
							width={32}
							height={32}
							priority
							className="rounded-md"
						/>
						<span className="font-bold text-lg">CriCriCri</span>
					</Link>
					<div className="items-center text-base font-base xl:gap-10 lg:flex gap-10 hidden">
						{isCharacterSheet ? (
							<>
								<Link href={`/characters/${params?.id}/sheet`}>Sheet</Link>
								<Link href={`/characters/${params?.id}/log`}>Log</Link>
							</>
						) : null}
					</div>
				</div>
			</div>
		</nav>
	);
}
